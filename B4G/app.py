...
# Route für die Großschreibung der Benutzernamen mit Map
@app.route('/uppercase_names', methods=['GET'])
def get_uppercased_names():
    uppercased_names = list(map(lambda user: user.username.upper(), registered_users))
    return jsonify({'uppercase_names': uppercased_names})


# Route für die Auswahl von Benutzern mit Filter
@app.route('/long_usernames', methods=['GET'])
def get_long_usernames():
    filtered_users = list(filter(lambda user: len(user.username) > 8, registered_users))
    long_usernames = [{'username': user.username, 'email': user.email} for user in filtered_users]
    return jsonify({'long_usernames': long_usernames})


# Route für die Konkatenation von Benutzernamen mit Reduce
@app.route('/concatenated_names', methods=['GET'])
def get_concatenated_names():
    concatenated_names = reduce(lambda acc, user: acc + user.username + ', ', registered_users, 'Namen: ')
    return jsonify({'concatenated_names': concatenated_names})
