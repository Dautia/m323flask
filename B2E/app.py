...
def register_user_with_binary_search(username, email, search_function):
...
# Benutzerregistrierung mit Binärer Suche über die gespeicherte Funktion
@app.route('/register', methods=['POST'])
def register_user():
    data = request.get_json()
    username, email = data['username'], data['email']

    response, status_code = register_user_with_binary_search(username, email, binary_search_users)
